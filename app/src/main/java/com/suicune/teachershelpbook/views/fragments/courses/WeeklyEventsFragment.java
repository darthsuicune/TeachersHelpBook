...
public class WeeklyEventsFragment extends Fragment {
	...
	public void updateDate(DateTime currentDate) {
		...
		if (dateView != null) {
			dateView.setText(String.format("%d/%d/%d - %d/%d/%d", startOfWeek.getDayOfMonth(),
					startOfWeek.getMonthOfYear(), startOfWeek.getYear(),
					endOfWeek.getDayOfMonth(), endOfWeek.getMonthOfYear(),
					endOfWeek.getYear()));
		}
		loadEvents();
	}
